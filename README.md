...
| 0.10        | 24/04/2025    | Alessandra Becerra, Leonardo Prieto, Leandro Contreras, Héctor Ríos, Gabriel Gordon             | Correcciones de nombres de imágenes; añadido ubiquitous language; sistemas de navegación; diagramas de componentes; perfil de equipo; diagrama de clases de la app          |
...
|------------------|--------|--------------|---------------------------|----------------------------|
| EP01 | Monitoreo en Tiempo Real de Maquinaria | El personal técnico y supervisor monitorea en tiempo real el estado operativo de las máquinas para detectar anomalías sin inspecciones físicas. | Dado que el sistema recibe datos en tiempo real<br>Cuando el usuario accede al panel de monitoreo<br>Entonces el sistema muestra métricas de operación actualizadas.<br><br>Dado que se selecciona una máquina específica<br>Cuando se accede a su detalle<br>Entonces se visualiza su rendimiento sin refrescar la página. | 01 |
| US01 | Visualización en tiempo real de métricas operativas por máquina | Como técnico de mantenimiento, quiero visualizar en tiempo real las métricas operativas de cada máquina, para detectar anomalías sin realizar inspecciones físicas. | Dado que el técnico accede al sistema<br>Cuando selecciona una máquina<br>Entonces visualiza las métricas operativas actualizadas.<br><br>Dado que ocurre una anomalía<br>Cuando el sistema detecta un comportamiento fuera del rango normal<br>Entonces se indica mediante los datos en pantalla. | 01 |
| US02 | Panel de control general para monitoreo de máquinas | Como jefa de planta, quiero tener una vista general del estado de todas las máquinas desde un panel principal, para tener el control total de la planta. | Dado que la jefa de planta accede al sistema<br>Cuando se carga el panel de monitoreo<br>Entonces observa el estado actualizado de todas las máquinas.<br><br>Dado que ocurre un cambio en el estado de alguna máquina<br>Cuando se actualiza la información<br>Entonces el sistema lo refleja en el panel sin intervención del usuario. | 01 |
...
| US15 | Almacenamiento histórico de controles de calidad | Como inspector de calidad, quiero almacenar los resultados de los controles realizados en producción, para mantener un registro histórico. | Dado que registra datos de un lote inspeccionado<br>Cuando guarda los datos<br>Entonces el sistema almacena correctamente con su clasificación.<br><br>Dado que desea consultar un lote pasado<br>Cuando busca por número o fecha<br>Entonces el sistema muestra los resultados correspondientes. | 03 |
...
| US19 | Análisis para prevenir fallas | Como ingeniero de mantenimiento, quiero que el sistema prediga fallas de maquinaria, para intervenir antes de que ocurran. | Dado que el sistema analiza datos históricos<br>Cuando detecta patrones similares a fallas pasadas<br>Entonces genera una alerta con grado de riesgo y recomendación.<br><br>Dado que recibe datos en tiempo real<br>Cuando identifica comportamiento anómalo<br>Entonces emite una predicción anticipada de falla. | 04 |
...
| TS74 | Implementar Creación de Tareas de Máquina | Como desarrollador, quiero implementar la creación de nuevas tareas para las máquinas, para que puedan ser planificadas y asignadas. | Escenario 1<br>Dado que se recibe una solicitud para crear una nueva tarea de máquina con todos los datos requeridos (ej. ID de máquina, descripción, fecha programada, tipo de tarea)<br>Cuando el sistema valida los datos de entrada<br>Entonces la tarea es persistida en la base de datos y se devuelve un identificador único de la tarea.<br>Escenario 2: Intento de creación de tarea con datos inválidos<br>Dado que se recibe una solicitud para crear una tarea con un ID de máquina inexistente o datos faltantes<br>Cuando el sistema intenta validar los datos<br>Entonces la tarea no es creada, se registra el error y se devuelve un mensaje de validación apropiado. | 04 |
| TS75 | Desarrollar Lógica para Asignación de Tareas a Máquinas | Como desarrollador, quiero desarrollar la lógica para asignar tareas a máquinas específicas, asegurando la trazabilidad de qué máquina tiene qué tarea pendiente. | Escenario 1<br>Dado que existe una tarea con un ID válido y un ID de máquina válido<br>Cuando se realiza una operación de asignación<br>Entonces la tarea se asocia correctamente a la máquina en la base de datos y se actualiza su estado si es necesario.<br>Escenario 2<br>Dado que se intenta asignar una tarea a un ID de máquina que no existe en el inventario<br>Cuando la lógica de asignación se ejecuta<br>Entonces la asignación es rechazada, se registra un error y la tarea no se modifica. | 04 |
| TS76 | Implementar Modificación de Tareas Existentes | Como desarrollador, quiero implementar la funcionalidad para modificar los detalles de las tareas de máquina existentes, para permitir ajustes en la planificación. | Escenario 1<br>Dado que existe una tarea con un ID válido y se recibe una solicitud con nuevos datos<br>Cuando el sistema valida los datos de entrada<br>Entonces la tarea es actualizada en la base de datos y se devuelve un código de estado 200.<br>Escenario 2<br>Dado que se intenta modificar una tarea con un ID inexistente o con datos de actualización no válidos<br>Cuando la lógica de modificación se ejecuta<br>Entonces la tarea no es modificada, se registra el error y se devuelve un mensaje de error apropiado. | 04 |
| TS77 | Implementar Eliminación Lógica de Tareas | Como desarrollador, quiero implementar la eliminación lógica de tareas (marcar como inactiva), para mantener un historial de auditoría sin borrar permanentemente los registros. | Escenario 1<br>Dado que existe una tarea con un ID válido y se recibe una solicitud DELETE<br>Cuando el sistema procesa la solicitud<br>Entonces la tarea es marcada como "inactiva" o "eliminada" en la base de datos, pero no es borrada físicamente, y se devuelve un código de estado 204.<br>Escenario 2<br>Dado que se intenta eliminar una tarea con un ID que no existe<br>Cuando la lógica de eliminación se ejecuta<br>Entonces el sistema devuelve un código de estado 404 (Not Found) y no realiza ninguna modificación. | 04 |
...
| US17 | Ajuste de turnos según demanda de producción | Como supervisor, quiero ajustar los turnos de trabajo según la demanda de producción, para optimizar el uso del personal. | Dado que la demanda aumenta<br>Cuando revisa el análisis de producción<br>Entonces actualiza los turnos en el sistema.<br><br>Dado que la demanda disminuye<br>Cuando reduce la cantidad de operarios<br>Entonces el sistema notifica automáticamente al personal afectado. | 05 |
| US21 | Dashboard unificado con KPIs clave | Como director de operaciones, quiero ver los KPIs clave de la planta en un solo dashboard, para tener una visión clara del desempeño. | Dado que accede al dashboard de indicadores<br>Cuando selecciona un periodo<br>Entonces el sistema muestra eficiencia, disponibilidad y calidad gráficamente.<br><br>Dado que filtra por área de producción<br>Cuando aplica el filtro<br>Entonces el sistema actualiza los KPIs mostrando solo los datos del área seleccionada. | 05 |
| US26 | Mantenimiento ante detección de retrasos | Como técnico de mantenimiento, quiero que el sistema reprograme automáticamente tareas retrasadas, para asegurar que se realicen a tiempo. | Dado que una tarea no se completó a tiempo<br>Cuando el sistema lo detecta<br>Entonces la reprograma automáticamente a la siguiente fecha disponible.<br><br>Dado que hay una nueva programación<br>Cuando se actualiza<br>Entonces el equipo es notificado con la nueva fecha de ejecución. | 05 |
| US32 | Registro y evaluación de paradas | Como jefe de mantenimiento, quiero registrar y evaluar las paradas programadas, para mejorar la planificación del mantenimiento. | Dado que programa una parada<br>Cuando la registra<br>Entonces el sistema la clasifica como programada y la muestra en el calendario.<br><br>Dado que solicita un informe<br>Cuando selecciona el mes<br>Entonces el sistema muestra la proporción entre paradas programadas y no programadas. | 05 |
...
| TS52 | Registro de Auditoria de Acceso al Sistema | Como desarrollador, quiero registrar todos los accesos al sistema para mantener un log de audotira de seguridad | Escenario 1<br>Dado que un usuario inicia sesión en el sistema<br>Cuando se autentica correctamente<br>Entonces el sistema debe registrar la fecha, hora, IP y método de acceso en el log de auditorí<br>Escenario 2<br>Dado que un usuario intenta acceder con credenciales inválidas<br>Cuando el intento falla<br>Entonces el sistema debe registrar el evento de acceso fallido en los registros de auditoría. | 08 |
| EP09 | Experiencia de Usuario y Marketing de Plataforma | Los visitantes del sitio web estático (landing page) reciben una experiencia clara, visual y multilingüe para conocer los beneficios de la plataforma e interesarse en registrarse. | Dado que un visitante accede a la landing page<br>Cuando navega por sus secciones<br>Entonces visualiza información clara, adaptada a su idioma y sin distracciones.<br><br>Dado que desea recibir novedades<br>Cuando se registra en el formulario<br>Entonces el sistema muestra un mensaje de confirmación y guarda el contacto. | 09 |
| US24 | Tutoriales interactivos para nuevos usuarios | Como encargado de formación, quiero ofrecer tutoriales interactivos, para que los nuevos usuarios aprendan a usar la plataforma. | Dado que un nuevo usuario inicia sesión<br>Cuando accede por primera vez<br>Entonces el sistema le ofrece un tutorial interactivo.<br><br>Dado que quiere reforzar el aprendizaje<br>Cuando accede a la sección de ayuda<br>Entonces puede ver el tutorial en cualquier momento. | 09 |
| US41 | Mostrar hero con mensaje claro | Como visitante, quiero ver un encabezado con título y subtítulo claros, para entender rápidamente el propósito de la plataforma. | Dado que el visitante accede a la página<br>Cuando se carga la landing<br>Entonces ve el nombre y subtítulo explicativo al inicio.<br><br>Dado que llega a esta sección desde otra vista<br>Cuando regresa al inicio<br>Entonces el hero vuelve a mostrarse correctamente. | 09 |
| US42 | Sección de características principales | Como visitante, quiero conocer las funcionalidades clave de la plataforma, para decidir si me interesa usarla. | Dado que el visitante visualiza la sección de características<br>Cuando está visible<br>Entonces ve los íconos, títulos y descripciones de cada funcionalidad.<br><br>Dado que quiere más información<br>Cuando da clic en una funcionalidad<br>Entonces el sistema le muestra más detalles si están disponibles. | 09 |
| US43 | Formulario de Contacto | Como usuario, quiero un formulario sencillo para dejar mi email, para recibir novedades sobre el lanzamiento. | Dado que completa su email<br>Cuando hace clic en 'Suscribirse'<br>Entonces el sistema muestra el mensaje '¡Gracias por suscribirte!'.<br><br>Dado que el email es inválido<br>Cuando intenta registrarse<br>Entonces el sistema muestra un mensaje de error. | 09 |
| US44 | Navegación fija | Como usuario, quiero un menú de navegación fijo, para acceder rápidamente a las secciones de la landing page. | Dado que el usuario hace scroll<br>Cuando llega al punto donde el header se oculta<br>Entonces el menú permanece visible arriba.<br><br>Dado que cambia de sección<br>Cuando usa el menú<br>Entonces el sistema lo dirige a la sección correspondiente sin recargar la página. | 09 |
| US45 | Diseño responsive | Como usuario en móvil, quiero que la landing page se adapte a pantallas pequeñas, para navegar cómodamente sin hacer zoom. | Dado que usa un dispositivo con ancho menor a 768px<br>Cuando accede a la landing<br>Entonces el layout se adapta y organiza los contenidos en una sola columna.<br><br>Dado que rota el dispositivo<br>Cuando cambia de orientación<br>Entonces el contenido se reorganiza sin perder formato. | 09 |
| US46 | Soporte de dos idiomas | Como usuario internacional, quiero cambiar la interfaz entre español e inglés, para usar la plataforma en mi idioma preferido. | Dado que accede al selector de idioma<br>Cuando elige 'English' o 'Español'<br>Entonces todo el texto se actualiza en el idioma elegido.<br><br>Dado que regresa al sitio<br>Cuando vuelve a ingresar<br>Entonces el sistema recuerda el idioma previamente seleccionado. | 09 |
| TS39 | Crear tutorial interactivo | Como desarrollador, quiero crear un tutorial interactivo para guiar a los nuevos usuarios, para que aprendan fácilmente. | Dado que el nuevo usuario inicia el tutorial<br>Cuando navega por los pasos<br>Entonces el sistema muestra instrucciones y ejemplos.<br><br>Dado que avanza en el tutorial<br>Cuando completa un paso<br>Entonces el sistema guarda su progreso y le permite retomarlo luego. | 09 |
| EP10 | Calidad y Trazabilidad de Producción | Los inspectores y administradores de seguridad aseguran la trazabilidad de los procesos mediante registros de calidad y auditorías de accesos. | Dado que se realiza una inspección o un acceso al sistema<br>Cuando se registra el evento<br>Entonces el sistema guarda la información con detalles y marca de tiempo.<br><br>Dado que se necesita revisar el historial<br>Cuando se realiza una búsqueda por lote o usuario<br>Entonces el sistema muestra todos los registros disponibles. | 10 |
| US23 | Auditoría de accesos a la plataforma | Como administrador de seguridad, quiero auditar los accesos a la plataforma, para garantizar que solo accedan usuarios autorizados. | Dado que accede al panel de auditoría<br>Cuando filtra por fecha<br>Entonces el sistema muestra los accesos con hora, usuario y actividad.<br><br>Dado que se accede fuera del horario permitido<br>Cuando el sistema detecta el evento<br>Entonces lo marca como inusual y notifica al administrador. | 10 |
| TS40 | Almacenar Resultados de Control de Calidad | Como desarrollador, quiero almacenar los resultados de las inspecciones de calidad, para garantizar la trazabilidad de los lotes. | Dado que el inspector registra resultados<br>Cuando se guardan los datos<br>Entonces el sistema los asocia al lote y producto correspondiente.<br><br>Dado que se requiere consultar una inspección<br>Cuando se busca por lote o producto<br>Entonces el sistema muestra los resultados almacenados. | 10 |
| EP11 | Gestión de Perfil y Personalización    | Inspectores, administradores y usuarios finales gestionan sus datos de perfil, controles de seguridad y preferencias de visualización para asegurar la trazabilidad de cambios y la comodidad de uso. | Dado que el usuario realiza cambios en su perfil o preferencias<br>Cuando guarda o consulta sus datos<br>Entonces el sistema registra y muestra la información correcta con marca de tiempo.                       |   11   |
|TS47 | Almacenar Resultados de Control de Calidad | Como desarrollador, quiero almacenar los resultados de las inspecciones de calidad, para garantizar la trazabilidad de los lotes.                                                               | Dado que el inspector registra resultados<br>Cuando se guardan los datos<br>Entonces el sistema los asocia al lote y producto correspondiente.<br><br>Dado que se requiere consultar una inspección<br>Cuando se busca por lote o producto<br>Entonces el sistema muestra los resultados almacenados. |   10   |
| US48 | Visualizar perfil de usuario           | Como usuario autenticado, quiero ver mi perfil con mis datos básicos, estado de membresía y tema activo, para comprobar rápidamente mi información y configuración.                            | Dado que el usuario ha iniciado sesión<br>Cuando entra en “Mi perfil”<br>Entonces ve su nombre completo, email, teléfono, estado de membresía (Activo/Inactivo) y el tema (Claro/Oscuro) en etiquetas destacadas. |    11   |
...
| 2 | TS75 | Desarrollar Lógica para Asignación de Tareas a Máquinas | Como desarrollador de backend, quiero desarrollar la lógica para asignar tareas a máquinas específicas, asegurando la trazabilidad de qué máquina tiene qué tarea pendiente. | 2 |
| 3 | TS77 |Implementar Eliminación Lógica de Tareas | Como desarrollador de backend, quiero implementar la eliminación lógica de tareas (marcar como inactiva), para mantener un historial de auditoría sin borrar permanentemente los registros. | 2 |
| 4  | US48          | Visualizar perfil de usuario             | Como usuario autenticado, quiero ver mi perfil con mis datos básicos, estado de membresía y tema activo, para comprobar rápidamente mi información y configuración.                        | 3            |
|   | US51          | Cambiar contraseña                        | Como usuario preocupado por la seguridad, quiero iniciar un flujo de cambio de contraseña desde mi perfil, para mantener mi cuenta protegida en todo momento.                              | 3            |
| 5 | TS35   | Generar Reporte de Producción   | Como desarrollador, quiero crear la funcionalidad para generar reportes de producción, para que los gerentes analicen el desempeño.  | 3 | 
| 6 | TS40   | Almacenar Resultados de Control de Calidad   | Como desarrollador, quiero almacenar los resultados de las inspecciones de calidad, para garantizar la trazabilidad de los lotes.  | 3 | 
| 7 | US41            | Mostrar hero con mensaje claro      | Como visitante, quiero ver un encabezado con título y subtítulo claros, para entender rápidamente el propósito de la plataforma.  | 3 | 
| 8 | US44            | Navegación fija                     | Como usuario, quiero un menú de navegación fijo, para acceder rápidamente a las secciones de la landing page.  | 3  |
| 9 | US46            | Soporte de dos idiomas              | Como usuario internacional, quiero cambiar la interfaz entre español e inglés, para usar la plataforma en mi idioma preferido.  | 3 |
| 10 | TS54 | Almacenar Preferencias de usuario | Como desarrollador, quiero implementar almacenamiento de preferencias para personalizar la experiencia del usuario. | 3 |
| 11 | TS55 | Gestionar Configuracion de Temas | Como desarrollador, quiero implementar gestión de temas visuales para permitir personalización de la interfaz. | 3 |
| 12 | TS57 | Gestionar Datos de Perfil de Usuario | Como desarrollador, quiero implementar CRUD de datos de perfil para permitir actualización de información personal. | 3 |
| 13 | TS74 |  Implementar Creación de Tareas de Máquina | Como desarrollador de backend, quiero implementar la creación de nuevas tareas para las máquinas, para que puedan ser planificadas y asignadas. | 3 |
| 14 | TS76 | Implementar Modificación de Tareas Existentes | Como desarrollador de backend, quiero implementar la funcionalidad para modificar los detalles de las tareas de máquina existentes, para permitir ajustes en la planificación. | 3 |
| 15 | US05   | Reportes periódicos para evaluación del rendimiento   | CComo jefa de planta, quiero acceder a reportes periódicos del rendimiento de las máquinas, para tomar decisiones basadas en datos reales.  | 5  |
| 16 | US06   | Recomendaciones para mantenimiento preventivo   | Como técnico de mantenimiento, quiero recibir recomendaciones de mantenimiento preventivo, para asegurar el buen funcionamiento de las máquinas.  | 5  |
| 17 | US07   | Detección de paradas de máquinas y sus causas   | Como jefa de planta, quiero saber cuándo y por qué se detiene una máquina, para actuar rápidamente y evitar retrasos.  | 5  |
| 18 | US14   | Gestión de accesos basada en roles   | Como administrador del sistema, quiero asignar roles a los usuarios, para gestionar sus permisos de acceso eficientemente.  | 5  |
| 19 | US15   | Almacenamiento histórico de controles de calidad   | Como inspector de calidad, quiero almacenar los resultados de los controles realizados en producción, para mantener un registro histórico.  | 5 |
| 20 | US16   | Recordatorios automatizados para mantenimiento   | Como jefe de mantenimiento, quiero automatizar los recordatorios de mantenimiento preventivo, para garantizar que no se pasen por alto.  | 5 |
| 21 | TS30   | Recuperar Datos de Máquina vía API   | Como desarrollador, quiero recuperar los datos de una máquina específica mediante una solicitud GET, para poder mostrar su estado en la interfaz de usuario.  | 5  |
| 22 | TS31   | Enviar Alerta de Fallo de Máquina   | Como desarrollador, quiero implementar la lógica para enviar alertas cuando se detecte un fallo en una máquina, para notificar al personal de mantenimiento.  | 5  |
| 23 | TS33   | Almacenar Historial de Rendimiento   | Como desarrollador, quiero almacenar el historial de rendimiento de las máquinas, para facilitar su análisis posterior.  | 5  |
| 24 | TS39   | Crear Tutorial Interactivo   | Como desarrollador, quiero crear un tutorial interactivo para guiar a los nuevos usuarios, para que aprendan fácilmente.  | 5 |
| 25 | US42            | Sección de características          | Como visitante, quiero conocer las funcionalidades clave de la plataforma, para decidir si me interesa usarla.  | 5 |
| 26 | US43            | Formulario de suscripción           | Como usuario, quiero un formulario sencillo para dejar mi email, para recibir novedades sobre el lanzamiento.  | 5 |
| 27 | TS47          | Almacenar Resultados de Control de Calidad | Como desarrollador, quiero almacenar los resultados de las inspecciones de calidad, para garantizar la trazabilidad de los lotes.                                                            | 5            |
| 28     | US49          | Editar datos del perfil                  | Como usuario, quiero actualizar mis datos personales (nombre, email, teléfono) y mi preferencia de tema, para mantener mi información al día y personalizar la interfaz.                   | 5            |
| 29 | TS52 | Registro de Auditoria de Acceso al Sistema | Como desarrollador, quiero registrar todos los accesos al sistema para mantener un log de auditoria de seguridad. | 5 |
| 30 | TS53 | Gestionar Sesiones de Usuario | Como desarrollador, quiero implementar gestión de sesiones activas para controlar y limitar sesiones concurrentes por usuario. | 5 |
| 31 | TS56 | Implementar Cambio de Contraseña Seguro | Como desarrollador, quiero implementar cambio de contraseña con validaciones para mantener la seguridad de las cuentas de usuario. | 5 | 
| 32 | TS59 | Almacenar Historial de Mantenimiento | Como desarrollador, quiero implementar almacenamiento de historial de mantenimiento para mantener trazabilidad de todas las intervenciones. | 5 |
| 33 | TS63 | Gestionar Turnos de Trabajo | Como desarrollador, quiero implementar gestión de turnos para organizar la asignación de personal por horarios. | 5 |
| 34 | TS65 | Implementar Recordatorios Automáticos | Como desarrollador, quiero implementar sistema de recordatorios automáticos, para notificar tareas de mantenimiento programadas. | 5 |
| 35 | TS67 | Gestionar Alertas y Notificaciones | Como desarrollador, quiero implementar sistema de alertas, para notificar eventos críticos inmediatamente. | 5 |
| 36 | TS72 | Calcular KPIs Operativos | Como desarrollador, quiero implementar cálculo de KPIs para generar indicadores clave de rendimiento. | 5 |
| 37 | US01   | Visualización en tiempo real de métricas operativas por máquina   | Como técnico de mantenimiento, quiero visualizar en tiempo real las métricas operativas de cada máquina, para detectar anomalías sin realizar inspecciones físicas.  | 8 |
| 38 | US02   | Panel de control general para monitoreo de máquinas   | Como jefa de planta, quiero tener una vista general del estado de todas las máquinas desde un panel principal, para tener el control total de la planta.  | 8 |
| 39 | US03   | Alertas automáticas ante incidentes en maquinaria   | Como operador, quiero recibir notificaciones automáticas cuando se detecten incidentes en las máquinas, para poder actuar rápidamente.  | 8 |
| 40 | US04   | Consulta del historial de fallas y mantenimientos   | Como técnico de mantenimiento, quiero consultar el historial de fallas y mantenimientos, para prevenir futuras averías.  | 8 |
| 41 | US08   | Dashboard en tiempo real de eficiencia por máquina   | Como gerente de planta, quiero ver un dashboard con la eficiencia de cada máquina en tiempo real, para tomar decisiones estratégicas.  | 8 |
| 42 | US09   | Alertas sobre cuellos de botella en producción   | Como gerente de planta, quiero recibir alertas sobre cuellos de botella, para tomar medidas correctivas de inmediato.  | 8 |
| 43 | US10   | Visualización del estado funcional de máquinas a cargo   | Como operador técnico, quiero ver el estado de funcionamiento de las máquinas a mi cargo, para monitorearlas y actuar ante fallas.  | 8 |
| 44 | US11   | Reporte de anomalías observadas en máquinas   | Como operador técnico, quiero reportar anomalías observadas en las máquinas, para que el equipo técnico pueda solucionarlas.  | 8 |
| 45 | US12   | Acceso a API documentada  | Como proveedor tecnológico, quiero acceder a una API documentada, para integrar nuestras soluciones con la plataforma.  | 8 |
| 46 | US13   | Configuración de Webhooks para eventos críticos   | Como proveedor tecnológico, quiero configurar Webhooks para recibir eventos importantes, para sincronizar nuestras acciones.   | 8 |
| 47 | US17   | Ajuste de turnos según demanda de producción   | Como supervisor, quiero ajustar los turnos de trabajo según la demanda de producción, para optimizar el uso del personal.  | 8 |
| 48 | US18   | Integración de la plataforma con el ERP   | Como responsable de IT, quiero integrar la plataforma con el ERP, para centralizar la información y evitar duplicación de datos.  | 8 |
| 49 | US19   | Análisis para prevenir fallas   | Como ingeniero de mantenimiento, quiero que el sistema prediga fallas de maquinaria, para intervenir antes de que ocurran.  | 8 |
| 50 | US20   | Accesibilidad multiplataforma de la plataforma   | Como supervisor de planta, quiero acceder a la plataforma desde diferentes dispositivos, para monitorear la producción desde cualquier lugar.  | 8 |
| 51 | US21   | Dashboard unificado con KPIs clave   | Como director de operaciones, quiero ver los KPIs clave de la planta en un solo dashboard, para tener una visión clara del desempeño.  | 8 |
| 52 | US22   | Gestión eficiente de repuestos críticos   | Como responsable de almacén, quiero gestionar los repuestos críticos eficientemente, para asegurar su disponibilidad cuando se necesiten.  | 8 |
| 53 | US23   | Auditoría de accesos a la plataforma   | Como administrador de seguridad, quiero auditar los accesos a la plataforma, para garantizar que solo accedan usuarios autorizados.  | 8 |
| 54 | US24   | Tutoriales interactivos para nuevos usuarios   | Como encargado de formación, quiero ofrecer tutoriales interactivos, para que los nuevos usuarios aprendan a usar la plataforma.  | 8 |
| 55 | US25   | Comparación de eficiencia en producción por periodos   | Como gerente de producción, quiero acceder al historial completo de producción, para poder comparar la eficiencia en diferentes periodos.  | 8 |
| 56 | US26   | Mantenimiento ante detección de retrasos   | Como técnico de mantenimiento, quiero que el sistema reprograme automáticamente tareas retrasadas, para asegurar que se realicen a tiempo.  | 8 |
| 57 | US27   | Análisis histórico comparativo de datos de planta   | Como analista de producción, quiero comparar datos históricos entre plantas, para analizar el rendimiento y proponer mejoras.  | 8 |
| 58 | US28   | Gestión de fallas de Maquinas   | Como técnico de soporte, quiero gestionar las fallas de las máquinas en tiempo real, para minimizar el tiempo de inactividad en la planta.  | 8 |
| 59 | US29   | Comparación de rendimiento entre turnos   | Como supervisor de planta, quiero comparar el rendimiento entre turnos, para identificar el más productivo.  | 8 |
| 60 | US32   | Registro y evaluación de paradas  | Como jefe de mantenimiento, quiero registrar y evaluar las paradas programadas, para mejorar la planificación del mantenimiento.  | 8 | 
| 61 | TS34   | Calcular Recomendaciones de Mantenimiento   | Como desarrollador, quiero implementar un algoritmo que calcule recomendaciones de mantenimiento preventivo, para optimizar la planificación.  | 8 |
| 62 | TS36   | Mostrar Estado de la Máquina en Tiempo Real   | Como desarrollador, quiero mostrar el estado actual de las máquinas a los operadores, para que puedan supervisar su funcionamiento.  | 8 | 
| 63 | TS37   | Integrar Sensor de Temperatura vía API   | Como desarrollador, quiero integrar sensores de temperatura mediante API, para recibir y almacenar sus datos.  | 8 | 
| 64 | TS38   | Implementar Autenticación de Usuario   | Como desarrollador, quiero implementar autenticación de usuarios, para asegurar que solo los autorizados accedan a la plataforma.  | 8 | 
| 65 | US45            | Diseño responsive                   | Como usuario en móvil, quiero que la landing page se adapte a pantallas pequeñas, para navegar cómodamente sin hacer zoom.  | 8 | 
| 66 | TS58 | Gestionar Inventario de Máquinas | Como desarrollador, quiero implementar gestión de inventario de máquinas para mantener registro actualizado de todos los activos. | 8 |
| 67 | TS60 | Gestionar Repuestos y Componentes | Como desarrollador, quiero implementar gestión de repuestos para controlar stock y disponibilidad de componentes. | 8 |
| 68 | TS61 | Implementar Trazabilidad de Activos | Como desarrollador, quiero implementar trazabilidad completa de activos para seguir el ciclo de vida de cada máquina. | 8 |
| 69 | TS62 | Algoritmo de Programación de Mantenimiento | Como desarrollador, quiero implementar algoritmos de programación de mantenimiento Para optimizar la planificación de intervenciones. | 8 |
| 70 | TS64 | Calcular Demanda de Recursos | Como desarrollador, Quiero implementar cálculo de demanda de recursos, Para optimizar la asignación de personal y materiales. | 8 |
| 71 | TS66 | Implementar Monitoreo en Tiempo Real | Como desarrollador, quiero implementar monitoreo en tiempo real, para supervisar el estado actual de todas las máquinas. | 8 |
| 72 | TS68 | Procesar Datos de Sensores | Como desarrollador, quiero implementar procesamiento de datos IoT para recibir y procesar información de sensores. | 8 |
| 73 | TS69 | Implementar Sistema de Eventos | Como desarrollador, quiero implementar un sistema de eventos para manejar la comunicación entre componentes del sistema. | 8 | 
| 74 | TS70 | Generar Métricas de Rendimiento | Como desarrollador, quiero implementar generación de métricas para calcular indicadores de rendimiento automáticamente. | 8 |
| 75 | TS71 | Implementar Agregación de Datos | Como desarrollador, quiero implementar agregación de datos para procesar grandes volúmenes de información eficientemente. | 8 |
| 76 | TS73 | Generar Reportes Automáticos | Como desarrollador, quiero implementar generación automática de reportes para crear informes periódicos sin intervención manual. | 8 |
...
![ClassDiagram-DyeTex](./assets/FinalV%20ClassDiagram.png)
...
| Gordon Salas, Gabriel Fernando               | Silent343       | L                                     |                                        |                                                |                                       |                                                               |
| Prieto Mantari, Leonardo Fabrizzio Junior    | Wuux1           |                                      |                                        |                                                |        L                               |                                                               |
...
| VS07     | Visualizar equipo de desarrollo | T07 | Team View | Implementación de la sección para visualizar al equipo de desarrollo. | 2                | Gabriel Gordon    | Done  |
| VS08     | Visualizar Call to Action claro   | T08 | Call To Action| Implementación de un Call To Action  | 1.5              |Gabriel Gordon| Done  |
| VS09     | Visualizar información del producto | T09 | About The Product| Implementación de sección About The Product | 2                | Leandro Contreras | Done  |
| VS10     | Formulario de suscripción de pago | T10| Subscription form | Creación de formulario de suscripción | 3                | Hector Rios       | Done |
...
| **Sprint 2 Velocity**                 |     La cantidad de puntos estimados son 38                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Sum of Story Points**               |  En el sprint 2, se calcuaron 38 puntos en función a las user stories implementadas                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| ----------------------------------------- | --------------- | ----------------------------- | ---------------------------- | ------------------------ | ----------------------------- | ------------------------------ |
| Becerra Tejeda, Alessandra Nicole         | aleeBecerra     | L                             | C                            |                        |                             |                              |
| Contreras López, Leandro Saul             | WiDDsito        |                              | L                            | C                        |                              |                              |
| Gordon Salas, Gabriel Fernando            | Silent343       | C                         |                         |                     |       L                   |                          |
| Prieto Mantari, Leonardo Fabrizzio Junior | Wuux1           |                              |                           | L                       | C                             |                               |
| Rios Pacheco, Hector Javier               | Khafna09        |                              |                             |                         | C                             | L                              |
...
| Story ID | Story Title                                                 | Task ID | Task Title                          | Description                                                                         | Estimation (Hrs) | Assigned To        | Status |
| -------- | ----------------------------------------------------------- | ------- | ----------------------------------- | ----------------------------------------------------------------------------------- | ---------------- | ------------------ | ------ |
| US-02    | Vista general del estado de todas las máquinas              | T10     | Machine Overview Dashboard          | Desarrollo de panel principal con resumen visual del estado general de las máquinas | 4.5              | Gabriel Gordon     | Done  |
| US-03    | Notificaciones automáticas por incidentes                   | T11     | Incident Notification System        | Lógica para detectar fallos y enviar alertas automáticas al operador                | 3                | Leandro Contreras    | Done  |
| US-08    | Dashboard con eficiencia en tiempo real                     | T12     | Real-time Machine Efficiency Charts | Visualización de KPIs en tiempo real por máquina                                    | 5                | Gabriel Gordon     | Done  |
| US-10    | Estado de funcionamiento de máquinas asignadas              | T13     | Machine Status View for Operators   | Interfaz donde cada operador técnico puede monitorear sus máquinas asignadas        | 3                | Alessandra Becerra | Done  |
| US-11    | Reporte de anomalías observadas en máquinas              | T14     | Machine Anomalies View    | Interfaz donde se ve un reporte con anomalías en máquinas        | 8                | Hector Rios | Done  |
| US-14    | Asignar roles y permisos a usuarios                         | T15     | User Role Management Panel          | Panel para asignación de roles, permisos y gestión de acceso de usuarios            | 3              | Leonardo Prieto    | Done  |
| US-16    | Automatización de recordatorios de mantenimiento preventivo | T16     | Preventive Maintenance Reminder     | Sistema de alertas programadas para mantenimiento según frecuencia configurada      | 3.5              | Hector Rios      | Done  |
| US-25    |  Comparación de eficiencia en producción por periodos | T17     | Comparison of production efficiency    | Vista de eficiencia de producción por periodos      | 8              | Leandro Contreras      | Done  |
...
| Endpoint                                   | Acción                            | Método HTTP | Sintaxis de llamada                                  | Parámetros                                                                                                                             | Ejemplo de Response |
|-------------------------------------------|-----------------------------------|-------------|------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|---------------------|
| /machine/notifications                    | Obtener notificaciones            | GET         | /machine/notifications                               | Ninguno                                                                                                                                | `{ "id": 1, "message": "Maquina con exceso de temperatura", "dateTime": "15/05/2025", "textilMachine": "MáquinaTextil_001", "markAsRead": true }` |
| /machine/notifications                    | Crear notificación                | POST        | /machine/notifications                               | id (int), message (string), dateTime (string), textilMachine (string), markAsRead (bool)                                               | `{ "id": 8, "message": "Sobrecalentamiento crítico", "dateTime": "2025-06-21T10:00:00", "textilMachine": "MáquinaTextil_005", "markAsRead": false }` |
| /machine/monitoring                       | Obtener datos de monitoreo        | GET         | /machine/monitoring                                  | Ninguno                                                                                                                                | `{ "id": "0f8fad5b-d9cb-469f-a165-70867728950e", "time_spent": "03:10:00", "day_progress": 82.3, "failure_rate": 1.8, "temperature": 68.5, ... }` |
| /machine/monitoring                       | Crear entrada de monitoreo        | POST        | /machine/monitoring                                  | id (string), time_spent (string), day_progress (float), failure_rate (float), amount_failure (int), user_id (string), temperature (float), vibration (float), energy (float), speed (int) | `{ "id": "new_id", "time_spent": "01:00:00", "day_progress": 50.0, "failure_rate": 0.0, "amount_failure": 0, "user_id": "1", "temperature": 65.2, "vibration": 0.3, "energy": 200.0, "speed": 1000 }` |
| /machine/maintenance                      | Obtener tareas de mantenimiento   | GET         | /machine/maintenance                                 | Ninguno                                                                                                                                | `{ "id": "task_1", "machine_id": "1", "description": "Revisar motor", "status": "pending" }` |
| /machine/maintenance                      | Crear tarea de mantenimiento      | POST        | /machine/maintenance                                 | id (string), machine_id (string), description (string), status (string)                                                                | `{ "id": "task_new", "machine_id": "1", "description": "Actualizar firmware", "status": "pending" }` |
| /machine/monitoring/machine-configuration | Obtener configuración de máquina  | GET         | /machine/monitoring/machine-configuration            | Ninguno                                                                                                | `{ "id": "8d7e0f42-4e8f-41c0-8d2e-61a91adf908f", "name": "Weaving Machine A", "asset_type": "Weaving", "status": "Operational", ... }` |
| /machine/monitoring/machine-configuration | Crear configuración de máquina    | POST        | /machine/monitoring/machine-configuration            | id (string), machine_information_id (string), name (string), asset_type (string), status (string), number_machine (string), floor (string), zone (string), date_installation (string) | `{ "id": "new_machine_id", "machine_information_id": "monitoring_id", "name": "New Machine", "asset_type": "Weaving", "status": "Operational", "number_machine": "WM-0000", "floor": "1", "zone": "North", "date_installation": "2025-06-21" }` |
| /machine/data-analytics                   | Obtener datos analíticos          | GET         | /machine/data-analytics                              | Ninguno                                                                                                                                | `{ "id": "0f8fad5b-d9cb-469f-a165-70867728950e", "energy": 220.3, "speed": 1200, "vibration": 0.4, "failure_rate": 1.8 }` |
| /machine/data-analytics                   | Enviar datos analíticos           | POST        | /machine/data-analytics                              | id (string), energy (float), speed (int), vibration (float), failure_rate (float)                                                      | `{ "id": "data_001", "energy": 300.5, "speed": 1300, "vibration": 0.5, "failure_rate": 2.1 }` |
| /user/assign-user                         | Obtener lista de usuarios         | GET         | /user/assign-user                                    | Ninguno                                                                                                                                | `{ "id": "ffb9dc8c-2e58-47f2-b437-7e6d6b00159c", "name": "Hector", "email": "hectorRios@gmail.com", "role": "tecnico", "permission": "monitoreo" }` |
| /user/assign-user                         | Crear usuario                     | POST        | /user/assign-user                                    | id (string), name (string), email (string), phone (string), start_date (string), plant (string), role (string), permission (string), card_id (string), subscription_active (bool) | `{ "id": "new_user_id", "name": "Ana", "email": "ana@email.com", "phone": "900000000", "start_date": "01/07/2025", "plant": "quinta planta", "role": "supervisor", "permission": "admin", "card_id": "abc123", "subscription_active": true }` |
| /user/profile                             | Obtener perfil del usuario        | GET         | /user/profile                                        | Ninguno                                                                                                                                | `{ "id": "1", "full_name": "Hector Rios", "email": "hectorRios@gmail.com", "phone": "941234056", "membership_active": true, "theme": "light" }` |
| /user/profile                             | Actualizar perfil del usuario     | POST        | /user/profile                                        | id (string), full_name (string), email (string), phone (string), membership_active (bool), theme (string)                             | `{ "id": "1", "full_name": "Hector Rios", "email": "hectorRios@gmail.com", "phone": "941234056", "membership_active": true, "theme": "dark" }` |
...
| **Sprint Velocity**                 | Para este sprint velocity se han estimado 97  story points                                                                                                                                                                                                                                         |
| **Sum of Story Points**             | La suma de history points a llevar a cabo es 97 story points                                                                                                                                                                                                                                                    |
...
En este sprint estamos encargardos de realizar un API Restful mediante la implementación del backend en C# & .Net. A continuación se mostrará la tabla con las stories relacionadas y llevadas a cabo en este tercer sprint.

<hr>

| Story ID | Story Title                             | Task ID | Task Title                              | Description                                                                                                                           | Estimation (Hrs) | Assigned To     | Status |
|----------|------------------------------------------|---------|------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------|------------------|------------------|--------|
| US-09    | Alertas sobre cuellos de botella en producción       | T15     | Alerts about bottlenecks | Agregar condiciones para detectar cuellos de botella | 8              | Hector Rios       | Done   |
| TS-54    | Almacenar preferencias de usuarios       | T16     | Store User Preferences (Endpoint Deployment) | Desarrollar un endpoint que permita almacenar y actualizar las preferencias del usuario, como idioma, notificaciones o tema visual. | 4.5              | Hector Rios       | Done   |
| TS-57    | Gestionar Datos de Perfil de Usuario     | T17     | Manage User Profile Data (Endpoint Deployment) | Crear el endpoint para consultar y modificar la información del perfil del usuario, incluyendo validaciones y seguridad de acceso. | 3.5              | Leonardo Prieto   | To-Do  |
| TS-31    | Enviar Alerta de Fallo de Máquina        | T18     | Logic for failure notifications (Endpoints Deployment) | Implementar la lógica y endpoints para detectar fallos en máquinas y generar notificaciones automáticas a los usuarios responsables. | 5                | Gabriel Gordon     | Done   |
| TS-67    | Gestionar Alertas y Notificaciones       | T19     | General alert system (Endpoints Deployment) | Diseñar el sistema de endpoints para registrar, actualizar y consultar alertas y notificaciones generales del sistema.               | 5                | Gabriel Gordon     | Done   |
| TS-33    | Almacenar Historial de Rendimiento     | T20     | Basis for reports and analysis (Endpoints Deployment) | Desarrollar un endpoint para registrar y almacenar datos históricos de rendimiento de máquinas, con el fin de habilitar análisis posteriores y reportes. | 3                | Hector Rios         | In-Process   |
| TS-63    | Gestionar Turnos de Trabajo            | T21     | Personnel organization. (Endpoints Deployment)        | Crear un endpoint que permita la asignación, modificación y consulta de turnos laborales del personal técnico, asegurando la organización eficiente.   | 3.5              | Leonardo Prieto     | To-Do        |
| TS-58    | Gestionar Inventario de Máquinas       | T22     | Basic asset management. (Endpoints Deployment)        | Implementar el endpoint para gestionar el inventario de máquinas, incluyendo su registro, edición, baja lógica y estado operativo.                      | 3.5              | Gabriel Gordon      | Done         |
| TS-38    | Implementar Autenticación de Usuario   | T23     | Prerequisite for almost all interactions.             | Desarrollar los endpoints de autenticación (login, logout, verificación de tokens) para permitir el acceso seguro al sistema.                          | 4                | Leonardo Prieto     | To-Do        |
| TS-30    | Recuperar Datos de Máquina                 | T24     | Essential for monitoring. (Endpoints Deployment) | Implementar un endpoint para obtener datos operativos de una máquina específica, facilitando su monitoreo y diagnóstico.                          | 3                | Alessandra Becerra   | Done         |
| TS-36    | Mostrar Estado de la Máquina en Tiempo Real| T25     | Core of real-time monitoring.             | Desarrollar la lógica necesaria para visualizar en tiempo real el estado funcional de la máquina, incluyendo indicadores como temperatura y uso. | 5                | Gabriel Gordon        | In-Progress  |
| TS-37    | Integrar Sensor de Temperatura             | T26     | Necessary for data collection.            | Conectar e integrar el sensor de temperatura al sistema, asegurando la correcta lectura, transmisión y almacenamiento de los datos capturados.    | 3.5              | Alessandra Becerra   | In-Progress  |
| TS-68    | Procesar Datos de Sensores                 | T27     | Information processing for monitoring.    | Crear la lógica que permita procesar y filtrar los datos provenientes de sensores, transformándolos en información útil para monitoreo y análisis.| 4                | Gabriel Gordon        | In-Progress  |
| TS-70    | Generar Métricas de Rendimiento            | T28     | Visualizing metrics for dashboards.                      | Desarrollar la lógica que transforme los datos procesados en métricas claras y relevantes para su visualización en dashboards operativos.        | 4                | Leandro Contreras     | Done   |
| TS-71    | Implementar Agregación de Datos            | T29     | Processing volumes of data.                              | Implementar la funcionalidad que permita agrupar grandes volúmenes de datos por períodos o categorías para facilitar el análisis posterior.      | 4.5              | Alessandra Becerra    | Done   |
| TS-74    | Implementar Creación de Tareas de Máquina  | T30     | Essential for recording new activities on the machines   | Diseñar el endpoint que permita registrar nuevas tareas en las máquinas, incluyendo tipo de tarea, fecha y parámetros asociados.                  | 3                | Leandro Contreras     | Done   |
| TS-75    | Desarrollar Lógica para Asignación de Tareas a Máquinas  | T31    | Crucial for linking tasks to specific assets             | Implementar la lógica que permita asignar automáticamente o manualmente tareas a máquinas específicas, considerando disponibilidad y prioridad.      | 3                | Leandro Contreras   | Done   |
| TS-76    | Implementar Modificación de Tareas Existentes             | T32     | Essential for task planning adaptability                 | Desarrollar la funcionalidad que permita modificar tareas ya registradas, ajustando atributos como fecha, duración, responsable o descripción.       | 3                | Leandro Contreras   | Done   |
| TS-77    | Implementar Eliminación Lógica de Tareas                  | T33     | Important for traceability and auditing when deactivating tasks | Implementar la lógica de eliminación lógica (soft delete) para tareas, permitiendo su desactivación sin pérdida de información histórica o trazabilidad. | 3                | Leandro Contreras   | Done   |
...
|---|---|---|
| GET | `/api/v1/designandplanning/tasks` | Obtener todas las tareas |
| POST | `/api/v1/designandplanning/tasks` | Crear una nueva tarea |
| GET | `/api/v1/designandplanning/tasks/{id}` | Obtener una tarea por ID |
| DELETE | `/api/v1/designandplanning/tasks/{id}` | Eliminar una tarea |
| PUT | `/api/v1/designandplanning/tasks/{id}/name` | Actualizar el nombre de una tarea |
...
**DeviceConfiguration y MachineInformations**

**DeviceConfiguration – Configuración individual de dispositivos**

| Método | Endpoint | Descripción |
|---|---|---|
| POST | `/api/v1/assets/textile-machines/device-configuration` | Crear configuración de dispositivo |
| GET | `/api/v1/assets/textile-machines/device-configuration` | Obtener todas las configuraciones |
| PUT | `/api/v1/assets/textile-machines/device-configuration/{ipAddress}` | Actualizar configuración por IP |
| GET | `/api/v1/assets/textile-machines/device-configuration/by-ip/{ipAddress}` | Obtener configuración por dirección IP |

**MachineInformations – Información de máquinas**

| Método | Endpoint | Descripción |
|---|---|---|
| GET | `/api/v1/assets/textile-machines/machine-informations/{id}` | Obtener información por ID |
| PUT | `/api/v1/assets/textile-machines/machine-informations/{id}` | Actualizar información por ID |
| POST | `/api/v1/assets/textile-machines/machine-informations` | Crear nueva información de máquina |
| GET | `/api/v1/assets/textile-machines/machine-informations` | Obtener toda la información de máquinas |
...
| Método | Endpoint | Descripción |
|---|---|---|
| POST | `/api/v1/dashboard&analytics/machine/failure-counts/refresh` | Refrescar fallas por máquina |
| GET | `/api/v1/dashboard&analytics/machine/failure-counts` | Obtener todos los conteos |
| GET | `/api/v1/dashboard&analytics/machine/{machineId}/failure-counts` | Obtener conteo por máquina |

**MachineFailureRate – Tasa de fallos por máquina**

| Método | Endpoint | Descripción |
|---|---|---|
| POST | `/api/v1/dashboard&analytics/machine/failure-rates/refresh` | Refrescar tasa de fallos |
| GET | `/api/v1/dashboard&analytics/machine/failure-rates` | Obtener todas las tasas |
| GET | `/api/v1/dashboard&analytics/machine/{machineId}/failure-rates` | Obtener tasa por máquina |

**TaskDueStatus – Estado de vencimiento de tareas**

| Método | Endpoint | Descripción |
|---|---|---|
| POST | `/api/v1/dashboard&analytics/task/due-status/refresh` | Refrescar estado por máquina |
| GET | `/api/v1/dashboard&analytics/task/due-status` | Obtener estados para todas las máquinas |

**Maintenance, Planning Task y RequestInvoice**

**Maintenance – Gestión de mantenimientos**

| Método | Endpoint | Descripción |
|---|---|---|
| GET | `/api/v1/designandplanning/maintenances` | Listar todos los mantenimientos |
| POST | `/api/v1/designandplanning/maintenances` | Crear un nuevo mantenimiento |
| GET | `/api/v1/designandplanning/maintenances/{id}` | Obtener un mantenimiento por ID |
| DELETE | `/api/v1/designandplanning/maintenances/{id}` | Eliminar un mantenimiento |
| PUT | `/api/v1/designandplanning/maintenances/{id}/status` | Actualizar el estado de un mantenimiento |

**Planning Task – Gestión de tareas de planificación**

| Método | Endpoint | Descripción |
|---|---|---|
| GET | `/api/v1/designandplanning/planning/tasks` | Listar todas las tareas de planificación |
| POST | `/api/v1/designandplanning/planning/tasks` | Crear una nueva tarea de planificación |
| GET | `/api/v1/designandplanning/planning/tasks/{id}` | Obtener una tarea de planificación por ID |
| DELETE | `/api/v1/designandplanning/planning/tasks/{id}` | Eliminar una tarea de planificación |
| PUT | `/api/v1/designandplanning/planning/tasks/{id}/name` | Actualizar el nombre de una tarea de planificación |

**RequestInvoice – Gestión de solicitudes de factura**

| Método | Endpoint | Descripción |
|---|---|---|
| GET | `/api/v1/designandplanning/request-invoices` | Listar todas las solicitudes de factura |
| POST | `/api/v1/designandplanning/request-invoices` | Registrar una nueva solicitud de factura |
| GET | `/api/v1/designandplanning/request-invoices/{id}` | Obtener una solicitud de factura por ID |
| DELETE | `/api/v1/designandplanning/request-invoices/{id}` | Eliminar una solicitud de factura |
| PUT | `/api/v1/designandplanning/request-invoices/{id}` | Actualizar una solicitud de factura |
...
|---|---|---|---|
| **Tasks** | Get All Tasks | GET | `/api/v1/designandplanning/tasks` |
| | Create Task | POST | `/api/v1/designandplanning/tasks` |
| | Get Task by ID | GET | `/api/v1/designandplanning/tasks/{id}` |
| | Delete Task | DELETE | `/api/v1/designandplanning/tasks/{id}` |
| | Update Task Name | PUT | `/api/v1/designandplanning/tasks/{id}/name` |
| | Update Task Due Date | PUT | `/api/v1/designandplanning/tasks/{id}/duedate` |
| **Profiles** | Get Profile by ID | GET | `/api/v1/users/profiles/{profileId}` |
| | Create Profile | POST | `/api/v1/users/profiles` |
| | Get All Profiles | GET | `/api/v1/users/profiles` |
| **Device Configuration** | Create Device Configuration | POST | `/api/v1/assets/textile-machines/device-configuration` |
| | Get All Device Configurations | GET | `/api/v1/assets/textile-machines/device-configuration` |
| | Update Device Configuration | PUT | `/api/v1/assets/textile-machines/device-configuration/{ipAddress}` |
| | Get Device Configuration by IP | GET | `/api/v1/assets/textile-machines/device-configuration/by-ip/{ipAddress}` |
| **Machine Informations** | Get Machine Information by ID | GET | `/api/v1/assets/textile-machines/machine-informations/{id}` |
| | Update Machine Information | PUT | `/api/v1/assets/textile-machines/machine-informations/{id}` |
| | Create Machine Information | POST | `/api/v1/assets/textile-machines/machine-informations` |
| | Get All Machine Information | GET | `/api/v1/assets/textile-machines/machine-informations` |
| **Textile Machines** | Get Textile Machine by ID | GET | `/api/v1/assets/textile-machines/{id}` |
| | Update Textile Machine | PUT | `/api/v1/assets/textile-machines/{id}` |
| | Get Textile Machine by Name | GET | `/api/v1/assets/textile-machines/{name}` |
| | Get All Textile Machines | GET | `/api/v1/assets/textile-machines` |
| | Create Textile Machine | POST | `/api/v1/assets/textile-machines` |
| **Machine Failure Counts** | Refresh Failure Counts by Machine | POST | `/api/v1/dashboard&analytics/machine/failure-counts/refresh` |
| | Get All Textile Machine Failure Counts | GET | `/api/v1/dashboard&analytics/machine/failure-counts` |
| | Get Failure Count by Machine ID | GET | `/api/v1/dashboard&analytics/machine/{machineId}/failure-counts` |
| **Machine Failure Rate** | Refresh Failure Rate by Machine | POST | `/api/v1/dashboard&analytics/machine/failure-rates/refresh` |
| | Get All Textile Machine Failure Rates | GET | `/api/v1/dashboard&analytics/machine/failure-rates` |
| | Get Failure Rate by Machine ID | GET | `/api/v1/dashboard&analytics/machine/{machineId}/failure-rates` |
| **Task Due Status** | Refresh Task Due Status by Machine | POST | `/api/v1/dashboard&analytics/task/due-status/refresh` |
| | Get All Task Due Status | GET | `/api/v1/dashboard&analytics/task/due-status` |
| **Maintenance** | List All Maintenances | GET | `/api/v1/designandplanning/maintenances` |
| | Create New Maintenance | POST | `/api/v1/designandplanning/maintenances` |
| | Get Maintenance by ID | GET | `/api/v1/designandplanning/maintenances/{id}` |
| | Delete Maintenance | DELETE | `/api/v1/designandplanning/maintenances/{id}` |
| | Update Maintenance Status | PUT | `/api/v1/designandplanning/maintenances/{id}/status` |
| **Planning Task** | List All Planning Tasks | GET | `/api/v1/designandplanning/planning/tasks` |
| | Create New Planning Task | POST | `/api/v1/designandplanning/planning/tasks` |
| | Get Planning Task by ID | GET | `/api/v1/designandplanning/planning/tasks/{id}` |
| | Delete Planning Task | DELETE | `/api/v1/designandplanning/planning/tasks/{id}` |
| | Update Planning Task Name | PUT | `/api/v1/designandplanning/planning/tasks/{id}/name` |
| **Request Invoice** | List All Request Invoices | GET | `/api/v1/designandplanning/request-invoices` |
| | Register New Request Invoice | POST | `/api/v1/designandplanning/request-invoices` |
| | Get Request Invoice by ID | GET | `/api/v1/designandplanning/request-invoices/{id}` |
| | Delete Request Invoice | DELETE | `/api/v1/designandplanning/request-invoices/{id}` |
| | Update Request Invoice | PUT | `/api/v1/designandplanning/request-invoices/{id}` |
###### 5.2.3.7. Software Deployment Evidence for Sprint Review

Backend Deployment

> Nos logeamos y creamos nuestra base de datos en freesqldatabase.com
<img src="./assets/dplyBcnk1.png" width="700">

> Entramos a render, nos logueamos y accedemos a web service
<img src="./assets/dplyBcnk2.png" width="700">

> Ingresamos el codigo de Docker en nuestro repositorio para posibilitar la conexción con el deploy
<img src="./assets/dplyBcnk3.png" width="700">

> Ingresamos la configuración para la conexión con la base de datos que hemos creado
<img src="./assets/dplyBcnk4.png" width="700">

> Ingresamos los datos que usaremos para el deploy 
<img src="./assets/dplyBcnk5.png" width="700">
<img src="./assets/dplyBcnk6.png" width="700">

> Damos click en Deploy Web Service
<img src="./assets/dplyBcnk7.png" width="700">

> Una vez hecho el deploy podemos visualizar el link por el cual podremos ingresar a nuestra web
<img src="./assets/dplyBcnk8.png" width="700">
<img src="./assets/dplyBcnk9.png" width="700">
...
| Story ID | Story Title                             | Task ID | Task Title                              | Description                                                                                                                           | Estimation (Hrs) | Assigned To     | Status |
|----------|------------------------------------------|---------|------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------|------------------|------------------|--------|
| US07 | Detección de paradas de máquinas y sus causas | T18 | Downtime Detection Module | Implementación de lógica para detectar y registrar causas de paradas automáticas | 5 | Alessandra Becerra | To Do |
| US12 | Acceso a API documentada | T19 | API Documentation Access | Configurar y publicar la documentación técnica para la API de integración | 8 | Gabriel Gordon | Done |
| US18 | Integración de la plataforma con el ERP | T20 | ERP Integration Logic | Implementar conectores para sincronización de datos con el ERP | 8 | Leonardo Prieto | Done |
| US19 | Análisis para prevenir fallas | T21 | Predictive Failure Analysis | Lógica de análisis basado en datos históricos para anticipar fallos | 8 | Leandro Contreras | Done |
| US22 | Gestión eficiente de repuestos críticos | T22 | Critical Spare Parts Management | Módulo para alertar sobre stock bajo y vincular repuestos con órdenes de mantenimiento | 8 | Gabriel Gordon | Done |
| US26 | Mantenimiento ante detección de retrasos | T23 | Auto-Reschedule Delayed Tasks | Reprogramación automática de tareas de mantenimiento retrasadas | 8 | Alessandra Becerra | Done |
| US28 | Gestión de fallas de máquinas | T24 | Real-time Machine Error Handling | Detección y gestión de errores en tiempo real con generación automática de tickets | 8 | Gabriel Gordon | Done |
| US32 | Registro y evaluación de paradas | T25 | Scheduled Downtime Log | Registro de paradas programadas con posibilidad de análisis y reportes | 8 | Leandro Contreras | Done |
| US21 | Dashboard unificado con KPIs clave | T26 | Unified KPI Dashboard | Visualización centralizada de indicadores clave como eficiencia, disponibilidad, calidad | 8 | Leonardo Prieto | Done |
| US15 | Almacenamiento histórico de controles de calidad | T27 | Quality Control Historical Storage | Persistencia y consulta de controles de calidad por lote o producto | 5 | Leonardo Prieto | Done |
| US27 | Análisis histórico comparativo de datos de planta | T28 | Plant Data Historical Comparison | Comparativa de métricas de eficiencia y producción entre plantas | 8 | Alessandra Becerra | Done |
